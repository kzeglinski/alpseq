...
process sample_1000 {
    ...
    output:
    tuple val(sequence_id), path(reads), path("*_sample.fastq")

    script:
    """
    read_base_name=\$(basename "$reads" .extendedFrags.fastq.gz)
    seqkit sample -n 1000 --threads $task.cpus $reads -o "\${read_base_name}_sample.fastq"
    """
}