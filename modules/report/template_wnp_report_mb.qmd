...
#| results: "asis"
#| echo: false
purrr::walk(sort(unique(metadata$round))[-1], render_child)
```
...
cluster_pca_plot_data <- make_cluster_pca_data(enriched_clustered, as.numeric(params$panning_id), metadata, max_round = max(metadata$round), edit_nt_seq = FALSE)
plot_cluster_pca(cluster_pca_plot_data$pca_data, cluster_pca_plot_data$table_data, params$analysis_name, edit_nt_seq = FALSE)
...
diversity_100 <- cluster_pca_plot_data$table_data %>%
    ...
    select(cluster_lead, sequence_alignment)
top_100_diversity_nt_seq <- Biostrings::DNAStringSet(diversity_100$sequence_alignment)
...
enrichment_100 <- cluster_pca_plot_data$table_data %>%
    ...
    select(cluster_lead, sequence_alignment)
top_100_enrichment_nt_seq <- Biostrings::DNAStringSet(enrichment_100$sequence_alignment)
