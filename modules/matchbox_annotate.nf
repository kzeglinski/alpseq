...
process matchbox_annotate {
    ...
    output:
    tuple val(sample_id), path('*_mb_annotation.tsv'), emit: annotation
    ...
    echo "id	v_gene	cdr3_nt	cdr3_aa	full_seq_nt	full_seq_aa" > \${read_base_name}_mb_annotation.tsv

    # then run matchbox
    matchbox --script ${mb_scripts}/generate_reference_counts.mb $read_sample -e 0.2 --threads ${task.cpus} --args "references = '${igblast_databases}/databases/imgt_alpaca_ighv'" -o "."
    ...
    matchbox --script ${mb_scripts}/preprocess_references.mb reference.fasta --threads ${task.cpus} -e 0.2 > trimmed_reference.fasta

    # then process the data
    # (ideally, matchbox should also be able to add the header to this csv output)
    matchbox --script ${mb_scripts}/find_cdr3_rescue_allow_gly.mb $reads -e 0.4 --threads ${task.cpus} >> \${read_base_name}_mb_annotation.tsv
    """
}